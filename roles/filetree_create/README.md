...
| :------------------------------ | :--------------------- | :------- | :-------------- | :---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `controller_api_plugin`         | `ctrliq.ascender`   | yes      | str             | Full path for the controller_api_plugin to be used. <br/> Can have two possible values: <br/>&nbsp;&nbsp;- ctrliq.ascender.controller_api             # For the community Collection version <br/>&nbsp;&nbsp;- ctrliq.ascender.controller_api  # For the Red Hat Certified Collection version                                                                                                                                                                         |
...
- hosts: all
  ...
  roles:
    - ctrliq.ascender.filetree_create
...
- **Flatten files**:
  ...
  The output files are all located in the same directory. Each file contains a YAML list with all the objects belonging to the same object type. This output format allows to load all the objects both from the standard Ansible `group_vars` and from the `ctrliq.ascender.filetree_read` role.
...
```console
ansible-playbook ctrliq.ascender.flatten_filetree_create_output.yaml -e '{filetree_create_output_dir: /tmp/filetree_output}'
```
