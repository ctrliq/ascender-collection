...
`ansible-galaxy collection install -r tests/collections/requirements.yml` to be installed. Currently: `ctrliq.ascender` or `ctrliq.ascender` and `ctrliq.ascender`.
...
| :------------ | :-----------: | :------: | :---------- |
| `controller_api_plugin` | `ctrliq.ascender` | yes | Full path for the controller_api_plugin to be used. <br/> Can have two possible values: <br/>&nbsp;&nbsp;- ctrliq.ascender.controller_api             # For the community Collection version <br/>&nbsp;&nbsp;- ctrliq.ascender.controller_api  # For the Red Hat Certified Collection version |
...
- hosts: localhost
  ...
  roles:
    - role: ctrliq.ascender.filetree_read
    - role: ctrliq.ascender.object_diff
      vars:
